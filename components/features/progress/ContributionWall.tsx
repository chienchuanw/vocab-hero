          ...
          <div className="flex-1">
            <div className="flex gap-1 mb-2 text-xs text-muted-foreground">
              {monthPositions.map((pos, idx) => (
                <div
                  key={idx}
                  style={{ marginLeft: idx === 0 ? 0 : `${(pos.column - (monthPositions[idx - 1]?.column || 0)) * 12}px` }}
                >
                  {pos.month}
                </div>
              ))}
            ...
            >
              {cells.map((cell, idx) => (
                <div
                  ...
                  className={`w-2.5 h-2.5 rounded-sm ${getIntensityColor(cell.intensity)}`}
                  title={
                    cell.date.getTime() > 0
                      ? `${cell.date.toLocaleDateString()}: ${cell.wordsStudied} words, ${cell.timeSpentMinutes} min`
                      : ''
                  }
                />
              ))}
