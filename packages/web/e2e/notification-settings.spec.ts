...
test.describe('Notification Settings', () => {
  ...
  test('should persist notification settings across page reloads', async ({ page }) => {
    ...
    if (await studyReminderToggle.isVisible()) {
      await studyReminderToggle.isChecked().catch(() => false);
  ...
  test('should show notification preview', async ({ page }) => {
    ...
    if (await previewButton.isVisible()) {
      ...
      await expect(page.getByText(/test.*notification.*sent|preview.*shown/i)).toBeVisible({
        timeout: 5000,
      });
    }
  });
  ...
  test('should reset notification settings to default', async ({ page }) => {
    ...
    if (await resetButton.isVisible()) {
      ...
      // Should show success message
      await expect(page.getByText(/reset.*success|default.*restored/i)).toBeVisible({
        timeout: 5000,
      });
    }
  });
